...
\end{aligned} 
\end{equation}
where \textit{Q} is the charge carried by the capacitor and \textit{V} is the applied DC voltage. The units for capacitance is farad \textit{F}. Applying Kirchoff's voltage law to this circuit when the source is a sinusoidal signal $Usin(\omega t)$ and using equation \eqref{eq:theory:capacitance} gives
...
The phase shift between current and voltage results in a pulsating power between the source and the load. For a purely resistive load with no phase shift, the power transferred to the load as given by \eqref{eq:theory:apparent_power} is always positive. 



\section{Voltage, current and power as complex numbers}
Physical quantities in a steady state alternating current (AC) power system are commonly expressed as a phasor in the complex plane. The voltage $U$ at time \textit{t} in an AC power grid is a sinusoidal signal $U = sin(\omega t)$ where $\omega$ is the angular frequency of the signal. Euler's formula states that a complex number $A$ can be expressed by
