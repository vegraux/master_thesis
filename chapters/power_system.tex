...
The active and reactive power flowing in a line can also be expressed as a complex number. The apparent power $S$ flowing in a line is defined to be
...
\end{figure}
Let N be the set of all buses in an electrical power system. Each branch is defined by the two buses $(i,j) \in$ L, where $i,j \in$ N.
