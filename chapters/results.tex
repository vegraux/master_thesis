...
The reward function had an energy imbalance term during training that was meant to incentives the model to shift energy consumption. However, the main goal of the agent is to keep the values for current and voltage within safety margins. Therefore, it is interesting to see the rewards of the trained model when the energy imbalance term is removed. Figure ?? plots the rewards in a 200 hour episode for the trained agent and for a no-agent situation that does not change the consumption. In other words, it is only penalised for violating safety margins for current and voltage in the grid. The rewards are zero for the most part because the safety margins are generally satisfied. It is clear that the trained agent has learned a strategy that pulls current and voltage values closer to nominal values. Inspecting the rewards further reveals that the agent is mainly preventing violation of voltage margins in figure ??.

Although the behaviour looks promising, it is still necessary to test the agent in several episodes. The trained agent was tested for 10 episodes, each consisting of 200 hours. Note that the solar data used varies from episode to episode. Therefore, the episodes use solar irradiance data ranging from February to October. The trained agent is tested for a range of solar irradiance profiles. All rewards corresponding to the hours when voltage margins are violated for the no-agent scenario are sorted and plotted in figure ??. The corresponding rewards given to the trained agent is also plotted
